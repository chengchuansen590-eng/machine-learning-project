...
class MACTN(BaseModel):
    def __init__(self, input_shape=(28, 512), output_shape: int = 9,
    ...
    def forward(self, x: torch.Tensor):
        seq_embed = self.forward_embed(x)  # [batch, seq_len, embed_dim]
        ...
        # 动态截取位置编码
        pos_embed = self.pos_embedding[:, :seq_len + 1]  # 保留[1, seq_len+1, embed_dim]
        embeddings += pos_embed

        tr_output = self.transformer(embeddings)
        return self.classifier(tr_output[:, 0])
