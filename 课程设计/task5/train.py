...
def train(model, loader, optimizer, loss_fn, device):
    ...
    for x, y in loader:
        ...
        optimizer.zero_grad()
        preds = model(x)  # [y1,y2,y3,y4] logits

        w = [0.1, 0.2, 0.3, 0.4]  # 深监督权重（越靠近最终输出权重越大）
        loss = w[0] * loss_fn(preds[0], y) + w[1] * loss_fn(preds[1], y) + w[2] * loss_fn(preds[2], y) + w[3] * loss_fn(
            preds[3], y)
...
def evaluate(model, loader, loss_fn, device):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            w = [0.1, 0.2, 0.3, 0.4]  # 深监督权重（越靠近最终输出权重越大）
            loss = w[0] * loss_fn(preds[0], y) + w[1] * loss_fn(preds[1], y) + w[2] * loss_fn(preds[2], y) + w[
                3] * loss_fn(
                preds[3], y)
